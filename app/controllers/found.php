...
class Found extends  CI_Controller{
	...
	//文章列表功能
   	function index(){
	   	if (isset($_GET['tid'])){
			...
			$cate="1";
	   		...
	   		$config['page_url'] = site_url('found').'?tid='.$tid.'&page=';  
	   ...
	   //$page = $this->uri->segment(3, 1);
		...
		if (!isset($post)){
			...
			$this->load->view('found/found_list',$data);
		}else{
			//分页
			//$sql="SELECT COUNT(*) AS count FROM post WHERE category=1";
	...
	function content(){
		...
		if (!isset($post)){
			...
			$this->load->view('found/found_page',$data);
		}
	}
	...
	function edit(){
		...
		$this->load->view('found/found_edit',$editdata);
		}
