    ...
    <div class="content">
    	...
    	<div class="list">
        	<div class="find_title font_heiti">
            	<a href="<?php echo  site_url('found/')?>" <?php if (isset($link_str)){?>style="width:640px;"<?php }?>>招领信息</a>
               <?php if (isset($link_str)) {echo $link_str;}else {?><a href="<?php echo  site_url('found/') ;?>" target="_blank"  class="go_another">=>去看看遗失信息</a><?php }?>
            </div>
            
           <ul class="detail">
           		<?php if (isset($post)){?>
	           		<li><span class="detail_t">物品名称：</span><span class="detail_c"><?php echo $post['title']?></span></li>
	           		<li><span class="detail_t">拾获地点：</span><span class="detail_c"><?php echo $post['place']?></span></li>
	           		<li><span class="detail_t">拾获时间：</span><span class="detail_c"><?php echo $post['time']?></span></li>
		           		...
		           		<?php if (isset($contact)) { ?>
			           		<?php echo $contact; if (isset($captcha_error)) if ($captcha_error){?>
			           			<?php echo  form_open(current_url());?>
				           			<li><div class="detail_t"><script language="javascript" type="text/javascript" src="<?php echo  site_url()?>imgauthcode/show_script/"></script></div><div class="detail_c" ><div style="color:red;font-size:12px;float:left;"><?php echo form_input('captcha');?>（输入验证码后回车查看联系信息.）</div></div></li>
				           		<?php echo form_close(); ?>
			           		<?php }?>
			           	<?php }else {?>
			           		<?php echo  form_open(current_url());?>
